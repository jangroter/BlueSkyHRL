...
def query_key_from_state(state):
    ...
    positions = state[:,:,3:5].clone()
    velocities = state[:,:,5:7].clone()
...
class RelativeAdditiveAttentionMultiHead(nn.Module):
    ...
    **Attention-Based Models for Speech Recognition**: https://arxiv.org/abs/1506.07503
    ...
    def forward(self, q_x, kv_x):
        ...
        # attempt with 10 highest weights instead of 10 closest
        # weights_squeezed = w.squeeze(-2)
        # topk_values, topk_indices = torch.topk(weights_squeezed, k=10, dim=-1)
        # topk_indices_expanded = topk_indices.unsqueeze(-1).expand(-1, -1, -1, -1, v.size(-1))

        # w = topk_values.view(b,t,h,1,10)
        # v = torch.gather(v, dim=-2, index=topk_indices_expanded)
